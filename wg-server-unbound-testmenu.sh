...
_tstSectionDNSChain() {
    ...
    _tst "Нет plain-text DNS наружу (порт 53 к внешним IP заблокирован нашим nft)" \
        bash -c "nft list ruleset 2>/dev/null | grep -q 'dport 53' \
            || ( nft list chain inet wg-policy forward 2>/dev/null | grep -q 'dport 53' ) \
...
_tstSectionAntiDPI() {
    ...
    if [ "${jitter}" = "yes" ]; then
        # [fix v28.24.25] Если юнит включён (enabled) но не запущен — запускаем автоматически
        if systemctl is-enabled wg-antidpi-jitter.service >/dev/null 2>&1 \
            && ! systemctl is-active wg-antidpi-jitter >/dev/null 2>&1; then
...
_tstSectionSecurity() {
    ...
    _tst "Ошибок в /var/log/wg-errors.log нет (пуст или отсутствует)" \
        bash -c "log=\"${WG_ERR_LOG:-/var/log/wg-server-trap.log}\"; \
            [ ! -f \"\${log}\" ] || [ ! -s \"\${log}\" ] \
            || ! grep -qvE '(read|_pause|ask|askYesNo)' \"\${log}\" 2>/dev/null"
