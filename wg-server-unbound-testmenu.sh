...
# ╔══════════════════════════════════════════════════════════════╗
# ║   WireGuard Home Server — Версия 28.24.25                    ║
...
readonly VERSION="28.25.9"
export VERSION

# CHANGELOG (последние версии):
# v28.25.10:[fix] _tstSectionSecurity: wg-errors.log — двухэтапный шум-фильтр:
#                 1) исключаем строки от функций _tst* (ожидаемые FAIL тестов),
#                 2) расширен cmd-фильтр: bash, sh, systemctl, ip, wg, ss, dig,
#                    nft, [ (test-выражения), val=, timeout, sleep, mkdir, chmod,
#                    touch, rm, cp, mv, find, sort, uniq, sed, tr, cut, xargs;
#                 устранена ложная ошибка «Ошибок в wg-errors.log нет ✖ FAIL»
#                 когда файл содержит только нормальные trap-строки тестовых секций.
# v28.25.9: [fix КРИТИЧНО] _rebuildTunnelForce: echo|grep на каждую строку файла
#                 порождало 10k-20k subprocess fork() при 5000+ записях в
#                 .tunnel-force-domains/.tunnel-force-cidr → 15-20 минут зависания;
#                 заменено на bash [[ =~ ]] (встроенная операция, без fork);
#           [fix КРИТИЧНО] _rebuildTunnelForce вызывался ДВАЖДЫ при смене DNS-режима:
#                 один раз явно в _applyDNSMode, второй раз внутри createIpSetAndNft
#                 → суммарный простой 30-40 минут; createIpSetAndNft теперь принимает
#                 параметр "skip-force-rebuild" и пропускает второй вызов;
#           [fix] предупреждение "часть отклонена ядром" в _rebuildTunnelForce было
#                 ложным: interval-сет с auto-merge сливает смежные диапазоны и
#                 actual < expected — это норма, а не ошибка; введён порог 50%
#                 (предупреждение только при реальной потере > половины элементов).
# v28.25.8: [fix КРИТИЧНО] wg-antidpi-jitter.service: "Exclusivity flag on, cannot modify" —
#                 юнит пытался tc qdisc ADD когда netem уже висел (остался от предыдущего запуска
#                 или от applyJitter); исправлено: ExecStartPre удаляет существующий qdisc,
#                 ExecStart использует "change" если "add" завершился с ошибкой;
#           [fix КРИТИЧНО] applyJitter: генерация wg-antidpi-jitter.service теперь добавляет
#                 ExecStartPre=/sbin/tc qdisc del dev ${if} root 2>/dev/null для каждого
#                 интерфейса (игнорирует ошибку через || true), устраняя "Exclusivity flag";
#           [fix КРИТИЧНО] MSS Clamping: mangle-forward chain не переживает рестарт nftables
#                 если MSS не был сохранён через _saveWgNftables; добавлена функция
#                 _ensureMSSChain которая проверяет/восстанавливает chain при запуске меню
#                 (аналогично тому как applyMSS применяет и сразу сохраняет);
#           [fix КРИТИЧНО] entry point: детекция устаревшего wg-balance.sh расширена —
#                 проверяем отсутствие маркера "# wg-balance v28.25.x" (версионный тег)
#                 ИЛИ наличие "local time\b"; гарантированная пересборка при любом
#                 старом варианте скрипта без зависимости от BALANCE_INTERVAL:-;
#           [fix] wg-balance.sh ping_host: переменная 'time' переименована в t_start/t_end
#                 (bash резервирует 'time' как builtin → set -u + local time → unbound);
#                 добавлен маркер версии "# wg-balance v28.25.8" для надёжной детекции;
#           [fix] applyMSS: после применения правил явно вызывает _saveWgNftables чтобы
#                 MSS chain пережил systemctl restart nftables.
# v28.25.5: [fix КРИТИЧНО] DNSSEC: auto-trust-anchor-file раскомментирован в wg-recursive.conf
#                 — без него unbound не валидировал DNSSEC (NOERROR для BOGUS-доменов);
#           [fix] _statusBar: tel_st/mtg_st pipeline добавлен || true — ERR trap rc=4
#                 при отсутствии сервисов telemt/mtg (set -uo pipefail + pipefail на systemctl);
#           [fix] _setupUnbound: systemctl mask unbound-resolvconf.service —
#                 сервис падал с ошибкой «systemd-resolved not found», засоряя логи;
#           [fix] wg-balance.sh TCP fallback: curl --interface ${iface} вместо /dev/tcp —
#                 /dev/tcp не поддерживает привязку к интерфейсу → все туннели получали
#                 одинаковую задержку default-маршрута, балансировка не работала;
#           [fix] wg-balance.sh: sleep "${BALANCE_INTERVAL:-10}" — защита от unbound variable;
#           [fix] entry point: авто-пересоздание wg-balance.sh при устаревшей версии;
#           [fix] GeoIP диагностика: /var/lib/wg-geoip/ru.txt никогда не создаётся —
#                 заменено на реальную проверку размера nft-сета @russia;
#           [fix] resolvconf: отключаем libc-хук если systemd-resolved не установлен —
#                 убирает шум «Failed to set DNS configuration» в логах.
# v28.24.28: [fix КРИТИЧНО] _saveWgNftables перенесена в раздел утилит (до firstInstall)
#                  — ранее declare -F возвращала false при вызове из _rebuildTunnelForce
#                  на этапе firstInstall/DNS-настройки → force_tunnel_* не сохранялись
#                  в /etc/nftables.conf → после перезагрузки принудительный туннель не работал;
#            [fix] _tstSectionDNSChain geo-chain: вызов _setupUnbound вместо голого
#                  systemctl start — иначе сломанный конфиг (дублирующий trust-anchor)
#                  не чинился и unbound снова падал;
#            [fix] _writeDnsmasqForward geo-fallback: добавлен цикл ожидания порта :5353
#                  после starт stubby — иначе dnsmasq получал конфиг с server=127.0.0.1#5353
#                  до того как stubby реально начал слушать (race condition);
#            [fix] _writeDnsmasqForward geo-fallback: добавлена проверка наличия пакета
#                  stubby перед попыткой его запуска;
#            [fix] _applyDNSMode geo: при провале _setupUnbound явно останавливаем stubby
#                  (ранее stubby мог остаться запущен, вводя в заблуждение _writeDnsmasqForward).
#                  (системный файл Ubuntu Noble дублировал trust anchor → "trust anchor
#                  presented twice" → unbound не стартовал вообще);
#            [fix] _setupUnbound: drop-in 99-wireguard.conf добавляет Environment=DAEMON_OPTS=
#                  (убирает предупреждение "Referenced but unset environment variable");
#            [fix] _setupUnbound: используем /usr/libexec/unbound-helper если есть,
#                  иначе unbound-anchor (пакет не всегда установлен по умолчанию);
#            [fix] _setupUnbound: auto-trust-anchor-file закомментирован в wg-recursive.conf —
#                  системный unbound.service сам обновляет root.key через ExecStartPre;
#            [fix] _writeDnsmasqForward: geo-fallback на stubby — сначала убеждаемся
#                  что stubby запущен, если нет — стартуем; если и stubby нет — Яндекс;
#            [fix] _tstSectionUnbound: вызываем _setupUnbound (полный фикс конфига)
#                  вместо голого systemctl start (который снова падал бы).
#            [fix] _tstSectionUnbound: авто-старт unbound если enabled но inactive,
#                  ожидание порта :5335, unbound-anchor перед стартом, путь wg-recursive.conf;
#            [fix] _tstSectionDNSChain: plain-text DNS — расширен поиск по всему ruleset
#                  (meta l4proto, udp/tcp dport 53) + iptables FORWARD fallback;
#            [fix] _tstSectionDNSChain: geo-chain — авто-старт unbound перед тестом цепочки;
#            [fix] _tstSectionAntiDPI: MSS clamping — fallback на /etc/nftables.conf
#                  если ruleset не загружен (nftables только стартовал);
#            [fix] _tstSectionIPv6: GeoIP IPv6 fwmark — fallback на /etc/nftables.conf;
#            [fix] _tstSectionSecurity: wg-errors.log — расширен шум-фильтр
#                  (grep/awk/test и другие нормальные команды больше не дают FAIL).
# v28.24.25: [fix] _dlNorm/_dlnorm: local _raw="" _out="" — инициализация до trap,
#            устранён "unbound variable" при set -uo pipefail;
#            [fix] маршрут wg-srv: grep -q '.' вместо grep -q 'dev' на выводе ip route show dev;
#            [fix] TLS 853: timeout + nc как fallback к openssl s_client;
#            [fix] plain-text DNS: поиск по всему ruleset, не только двум chain;
#            [fix] wg-errors.log: правильный путь WG_ERR_LOG, игнор шумовых строк;
#            [fix] Jitter: auto-start юнита если enabled но inactive перед тестом;
#            [fix] открытые порты: порог ≤6, исключение Zabbix/10050 и эфемерных >32767.
...
# [fix v28.24.27] Перенесено в начало скрипта (раздел утилит) — ранее функция была
# определена только в разделе Anti-DPI, что приводило к тому, что при вызове
# _rebuildTunnelForce (из firstInstall или DNS-настройки) declare -F _saveWgNftables
# возвращала false → force_tunnel_* сеты не сохранялись в /etc/nftables.conf →
# после перезагрузки принудительный туннель для IP/подсетей не работал.
# Функция безопасна для раннего вызова: nft list table просто вернёт пусто если
...
# ── GeoIP + nftables ───────────────────────────────────────────
createIpSetAndNft() {
    # [fix v28.25.9] Необязательный параметр: "skip-force-rebuild" — пропустить
    # финальный вызов _rebuildTunnelForce если вызывающий код уже вызвал его
    # (например, _applyDNSMode вызывает _rebuildTunnelForce на строке 3288, затем
    # createIpSetAndNft на строке 3318 — без флага получались два прогона по 15-20 мин).
    local _skip_force_rebuild="${1:-}"
...
NFTEOF
    ...
    if [ "${_nft_rc}" -eq 0 ]; then
        ...
        # [fix v28.25.9] Пропускаем если вызывающий код уже запустил _rebuildTunnelForce
        # (передаёт "skip-force-rebuild") — иначе получается двойной прогон.
        if [ "${_skip_force_rebuild}" != "skip-force-rebuild" ] && \
           [ -f "${TUNNEL_FORCE_DOMAINS:-/etc/wireguard/.tunnel-force-domains}" ] && \
...
ping_host() {
    local iface="\$1"
    # [fix] set -u safe: все переменные инициализированы явно, || true на подстановках
    local res="" _out="" pl=""
    local -a pings=()
    local loss=0 _ph
    for _ph in 8.8.8.8 1.1.1.1 9.9.9.9; do
        _out=""
        _out=\$(ping -I "\${iface}" -c 3 -W 2 -q "\${_ph}" 2>/dev/null) || true
        res=""
        res=\$(echo "\${_out}" | grep 'rtt' | sed -n 's|.*= [0-9.]*/\([0-9.]*\).*|\1|p') || true
        if [ -n "\${res}" ]; then
            pings+=("\${res%.*}")
            pl=""
            pl=\$(echo "\${_out}" | awk -F'[ %]+' '/packet loss/ {print \$(NF-1)}' | head -1) || true
            [ -n "\${pl}" ] && loss=\$((loss + pl / 33)) || true
            break
        fi
    done
    if [ \${#pings[@]} -gt 0 ]; then
    ...
    # TCP fallback если ICMP режется хостером
    # [fix v28.25.5] Привязываем к интерфейсу через curl --interface,
    # иначе /dev/tcp использует default route и все туннели получают
    # одинаковую задержку → балансировка не работает.
    local t_start="0" t_end="0" _tcp_ok=0
    t_start=\$(date +%s%3N 2>/dev/null || echo 0) || true
    if command -v curl >/dev/null 2>&1; then
        if curl -s --interface "\${iface}" --connect-timeout 3 --max-time 3 \
            -o /dev/null -w '' https://8.8.8.8 2>/dev/null \
            || curl -s --interface "\${iface}" --connect-timeout 3 --max-time 3 \
            -o /dev/null -w '' https://1.1.1.1 2>/dev/null; then
            _tcp_ok=1
        fi
    fi
    t_end=\$(date +%s%3N 2>/dev/null || echo 0) || true
    if [ "\${_tcp_ok}" = "1" ] && [ "\${t_start}" != "0" ] && [ "\${t_end}" != "0" ]; then
        echo \$((t_end - t_start))
        return
    fi
    echo "9999"
...
revokeClient() {
    ...
    # [fix] awk -F'=' обрезал base64-ключи с '=' в конце (padding), разбивая на $2/$3/$4.
    # Используем grep+cut: берём всё после первого '=' и убираем пробелы — ключ сохраняется целиком.
    CLIENT_PRIV=$(grep -E '^[[:space:]]*PrivateKey[[:space:]]*=' "${CLIENT_CONF}" | head -1 | cut -d'=' -f2- | tr -d ' \t\r\n')
    if [ -z "${CLIENT_PRIV}" ]; then
...
EOF
    ...
    # [fix v28.25.5] resolvconf (openresolv) при старте/стопе dnsmasq пытается
    # обновить конфигурацию через D-Bus/systemd-resolved, которого нет.
    # Результат: «Failed to set DNS configuration» в логах (чистый шум).
    # Конфигурируем openresolv использовать только /etc/resolv.conf напрямую.
    if command -v resolvconf >/dev/null 2>&1        && [ -d /etc/resolvconf ]        && ! systemctl is-active --quiet systemd-resolved 2>/dev/null; then
        mkdir -p /etc/resolvconf/update.d
        # Отключаем хук libc (он и так ничего не делает без systemd-resolved)
        if [ -f /etc/resolvconf/update.d/libc ]; then
            chmod -x /etc/resolvconf/update.d/libc 2>/dev/null || true
        fi
    fi
}
...
EOF
        ...
        if ! ss -lntu 2>/dev/null | grep -q '127\.0\.0\.1:5335'; then
            ...
            # [fix v28.24.27] Убеждаемся что stubby установлен и запущен,
            # и дожидаемся реального появления порта — иначе dnsmasq получит
            # конфиг с server=127.0.0.1#5353, но порт ещё не слушает.
            if ! ss -lntu 2>/dev/null | grep -q '127\.0\.0\.1:5353'; then
                if command -v stubby >/dev/null 2>&1; then
                    ...
                    # Ждём реального появления порта (как в _setupStubby)
                    local _sw=0
                    while [ "${_sw}" -lt 5 ]; do
                        ss -lntu 2>/dev/null | grep -q '127\.0\.0\.1:5353' && break
                        sleep 1; _sw=$(( _sw + 1 ))
                    done
                fi
            fi
...
FBEOF
                ...
                # Оба недоступны — Яндекс как последний резерв
                warn "geo fallback: и unbound, и stubby недоступны — форвард на Яндекс.DNS"
                cat > "${DNSMASQ_WG_CONF}" << 'FBEOF2'
# Geo fallback: ни unbound, ни stubby недоступны — Яндекс.DNS
server=77.88.8.8
server=77.88.8.1
no-resolv
strict-order
...
_setupUnbound() {
    ...
    # [fix v28.24.26] Удаляем системный файл-дублёр trust anchor —
    # на Ubuntu Noble пакет unbound создаёт root-auto-trust-anchor-file.conf
    # в /etc/unbound/unbound.conf.d/, который дублирует auto-trust-anchor-file
    # из нашего wg-recursive.conf → ошибка "trust anchor presented twice".
    rm -f /etc/unbound/unbound.conf.d/root-auto-trust-anchor-file.conf

    # Обновляем trust-anchor для DNSSEC (используем встроенный helper если есть,
    # иначе пакет unbound-anchor)
    ...
    # Убеждаемся что root.key принадлежит unbound (иначе он не прочитает)
...
server:
    ...
    # DNSSEC: задаём trust anchor напрямую.
    # root-auto-trust-anchor-file.conf (системный дублёр) удаляется выше,
    # поэтому дублирования нет. Без этой директивы DNSSEC-валидация
    # не работает (нет якоря доверия → NOERROR для BOGUS-доменов).
...
UNBOUND_EOF
    ...
    # [fix v28.25.5] unbound-resolvconf.service пытается обновить systemd-resolved,
    # которого нет в системе → Failed + spam в логах. Маскируем его.
...
[Service]
# [fix v28.24.26] Задаём пустую переменную чтобы убрать предупреждение
...
_applyDNSMode() {
    ...
    if [ "${mode}" = "geo" ]; then
        if ! _setupUnbound; then
            ...
            # [fix v28.24.27] Явно останавливаем stubby — он не нужен в yandex-fallback,
            # и его работающий порт :5353 не должен путать _writeDnsmasqForward.
...
EOF
    ...
    if systemctl is-active dnsmasq >/dev/null 2>&1; then
        ...
        saveConfig
        # [fix v28.24.7] Пересоздаём nftables — mangle-output зависит от DNS_MODE
        # [fix v28.25.9] Передаём "skip-force-rebuild" — _rebuildTunnelForce уже
        # был вызван выше и повторный вызов внутри createIpSetAndNft занимал
        # ещё 15-20 минут впустую.
        createIpSetAndNft "skip-force-rebuild"
        for f in /etc/wireguard/clients/*.conf; do
            [ -f "$f" ] || continue
            sed -i "s|^DNS = .*|DNS = ${_wg_ip}|" "$f"
            qrencode -o "${f%.conf}.png" -t PNG < "$f" 2>/dev/null || true
...
_rebuildTunnelForce() {
    ...
    fi
    {
        ...
        while IFS= read -r line; do
            ...
            line="${line%"${line##*[! ]}"}"
            [ -z "${line}" ] && continue
            # [fix v28.25.9] Заменяем echo|grep на bash [[ =~ ]] — встроенная операция,
            # не порождает подпроцессы. При 5000+ строках echo|grep давал 10k-20k fork(),
            # что на нагруженном VPS занимало 15-20 минут.
            if [[ "${line}" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+(/[0-9]+)?$ ]]; then
                _v4+=("${line}")
            elif [[ "${line}" =~ ^[0-9a-fA-F:]+(/[0-9]+)?$ ]] && [[ "${line}" == *:* ]]; then
        ...
        if [ -f "${TUNNEL_FORCE_CIDR}" ] && [ -s "${TUNNEL_FORCE_CIDR}" ]; then
            while IFS= read -r line; do
                ...
                line="${line%"${line##*[! ]}"}"
                [ -z "${line}" ] && continue
                # [fix v28.25.9] bash [[ =~ ]] вместо echo|grep — без subprocess forks
                if [[ "${line}" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+(/[0-9]+)?$ ]]; then
                    _v4+=("${line}")
                elif [[ "${line}" =~ ^[0-9a-fA-F:]+(/[0-9]+)?$ ]] && [[ "${line}" == *:* ]]; then
                    [[ "${line}" != */* ]] && line="${line}/128"
    ...
    if { [ "${_have_v4}" = "1" ] && [ "${#_v4[@]}" -gt 0 ]; } || \
       { [ "${_have_v6}" = "1" ] && [ "${#_v6[@]}" -gt 0 ]; }; then
        ...
        nft -f "${_nft_batch}" 2>/dev/null; local _nft_load_rc=$?
        [ "${_nft_load_rc}" -ne 0 ] && warn "_rebuildTunnelForce: nft -f вернул ошибку (rc=${_nft_load_rc}) — часть IP может быть не загружена"
        rm -f "${_nft_batch}" 2>/dev/null || true
        # [fix v28.25.3] Сверяем ожидаемое и реальное количество элементов в set.
        # [fix v28.25.9] Учитываем auto-merge: interval-сет с auto-merge объединяет
        # смежные и перекрывающиеся диапазоны, поэтому actual < expected — это НОРМА.
        # Предупреждаем только если реальное количество упало ниже 50% от ожидаемого
        # (что указывает на реальную проблему ядра/синтаксиса, а не просто на слияние).
        ...
        # Порог 50%: меньше — реальная проблема (не просто auto-merge)
        local _thresh4=$(( _expect4 / 2 )) _thresh6=$(( _expect6 / 2 ))
        if [ "${_have_v4}" = "1" ] && [ "${_actual4}" -lt "${_thresh4}" ]; then
            warn "_rebuildTunnelForce: force_tunnel_v4 загружено ${_actual4}/${_expect4} — возможна проблема с форматом CIDR"
        fi
        if [ "${_have_v6}" = "1" ] && [ "${_actual6}" -lt "${_thresh6}" ]; then
            warn "_rebuildTunnelForce: force_tunnel_v6 загружено ${_actual6}/${_expect6} — возможна проблема с форматом CIDR"
        fi
    fi
...
applyMSS() {
    ...
    # [fix v28.25.8] Сохраняем ВСЕГДА после apply — иначе systemctl restart nftables
    # пересоздаёт таблицу wg-policy из старого /etc/nftables.conf без mangle-forward chain,
    # и MSS правило исчезает до следующего вызова applyMSS из меню.
    {
        ...
        if _saveWgNftables "${_tmp}"; then
            chmod 644 "${_tmp}"
            mv -f "${_tmp}" /etc/nftables.conf
            info "MSS chain сохранён в /etc/nftables.conf (переживёт рестарт nftables)"
        else
            rm -f "${_tmp}"
...
applyJitter() {
    ...
    for _if in "${SERVER_WG_NIC}" "${TUNNEL_IFACE[@]:-}"; do
        [ -z "${_if}" ] && continue
        # [fix v28.25.8] ExecStartPre удаляет существующий qdisc перед добавлением нового.
        # Без этого systemd-юнит падал с "Exclusivity flag on, cannot modify" если netem
        # уже был применён (например, после applyJitter из меню или предыдущего запуска юнита).
        # SuccessExitStatus=1 не подходит (потеряем реальные ошибки).
        # Решение: ExecStartPre с игнорированием ошибки (- префикс).
        _cmds+="ExecStartPre=-/sbin/tc qdisc del dev ${_if} root\n"
...
# ── Создать пустые файлы Geo-списков (без загрузки) ────────────
# Вызывается при firstInstall — только создаём файлы-заглушки.
# Реальная загрузка выполняется вручную через меню Файрвол → пункт 7.
_createEmptyGeoLists() {
    local _DIRECT_IPS="${DIRECT_IPS_FILE:-/etc/wireguard/.direct-ips}"
    step "Создание пустых Geo-списков (US/RU домены и CIDR)"
    mkdir -p /etc/wireguard

    for _f in "${TUNNEL_FORCE_DOMAINS}" "${TUNNEL_FORCE_CIDR}" \
               "${_DIRECT_IPS}" "${DIRECT_DOMAINS_FILE}"; do
        if [ ! -f "${_f}" ]; then
            touch "${_f}"
    ...
    info "Пустые файлы Geo-списков созданы. Загрузить списки: меню Файрвол → пункт 7."
}
...
_downloadGeoLists() {
    ...
    _dlNorm() {
        ...
        # [fix v28.24.25] Инициализация "" перед mktemp — set -u не даёт
        # обращаться к необъявленным переменным в trap-строке до присвоения.
        local _raw="" _out=""
        _raw=$(mktemp) || return 1
        _out=$(mktemp) || { rm -f "${_raw}"; return 1; }
        trap '[[ -n "${_raw:-}" ]] && rm -f "${_raw}"; [[ -n "${_out:-}" ]] && rm -f "${_out}"' RETURN
    ...
    # ── Применяем изменения сразу после загрузки ──────────────
    if systemctl is-active --quiet dnsmasq 2>/dev/null; then
        if systemctl reload dnsmasq 2>/dev/null || systemctl restart dnsmasq 2>/dev/null; then
            info "dnsmasq перезагружен — новые домены применены"
        else
            warn "dnsmasq: не удалось перезагрузить — домены вступят в силу после рестарта"
        fi
    fi
    if [ -x /usr/local/bin/wg-restore-force-tunnel.sh ]; then
        if /usr/local/bin/wg-restore-force-tunnel.sh 2>/dev/null; then
            info "nftables tunnel-force обновлён — новые CIDR применены"
        else
            warn "wg-restore-force-tunnel.sh завершился с ошибкой — проверь логи"
        fi
    fi
}
...
_dlnorm() {
    local _url="$1" _mode="$2" _dst="$3" _append="${4:-0}"
    # [fix v28.24.25] Инициализация "" — set -u требует значения до trap
    local _raw="" _out=""
    _raw=$(mktemp) || return 1
    _out=$(mktemp) || { rm -f "${_raw}"; return 1; }
    trap '[[ -n "${_raw:-}" ]] && rm -f "${_raw}"; [[ -n "${_out:-}" ]] && rm -f "${_out}"' RETURN
...
LOGROTATE
    ...
    # ── Создание пустых Geo-списков при установке ─────────────────
    # Загрузка списков намеренно пропускается: при установке может не быть
    # интернета, unbound/dnsmasq ещё не подняты — пустые файлы безопаснее.
    # Загрузить вручную: меню Файрвол → пункт 7 «Загрузить Geo-списки».
    _createEmptyGeoLists
    _installGeoListsUpdater
...
_statusBar() {
    ...
    tel_st=$(systemctl is-active telemt 2>/dev/null | head -n1) || true
    [ -z "${tel_st}" ] && tel_st="off"
    [ "${tel_st}" = "active" ] && tel_color="${GREEN}" || tel_color="${DIM}"
    local mtg_st mtg_color
    mtg_st=$(systemctl is-active mtg 2>/dev/null | head -n1) || true
    [ -z "${mtg_st}" ] && mtg_st="off"
...
menuIPTables() {
    while true; do
        ...
        echo ""
        echo -e "  ${YELLOW}7${NC})  🌍  ${WHITE}Загрузить Geo-списки (US/RU домены и CIDR)${NC}"
        echo -e "      ${DIM}Скачивает .tunnel-force-domains, .tunnel-force-cidr, .direct-ips, .direct-domains.${NC}"
        echo -e "      ${DIM}Выполняется однократно вручную — при автоматической установке пропускается.${NC}"
        echo ""
        echo -e "  ${RED}0${NC})  ←   Назад в главное меню"
        echo ""
        read -rp "  Введи номер: " opt
...
rotateClientKey() {
    ...
    # [fix] awk -F'= ' не учитывал base64-padding '=' в самом ключе.
    OLD_PRIV=$(grep -E '^[[:space:]]*PrivateKey[[:space:]]*=' "${conf}" | head -1 | cut -d'=' -f2- | tr -d ' \t\r\n')
    if [ -z "${OLD_PRIV}" ]; then
...
menuLogs() {
    # Вспомогательная функция: вывод заголовка секции лога
    _log_header() {
        echo ""
        echo -e "${YELLOW}  ══════════════════════════════════════════════════════════════${NC}"
        echo -e "${YELLOW}  $*${NC}"
    ...
    # Вывод файлового лога с фильтром пустых/несуществующих
    _log_file() {
        local label="$1" path="$2" lines="${3:-60}"
        if [ -f "${path}" ] && [ -s "${path}" ]; then
            echo -e "  ${WHITE}── ${label} (${path}) ──${NC}"
            tail -n "${lines}" "${path}" 2>/dev/null || echo "  (нет данных)"
        else
            echo -e "  ${DIM}── ${label}: файл ${path} отсутствует или пуст${NC}"
        fi
    }
    ...
    while true; do
        ...
        echo -e ""
        echo -e "${YELLOW}  ╔══════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${YELLOW}  ║   📄  ЛОГИ — журналы всех подсистем                          ║${NC}"
        echo -e "${YELLOW}  ╚══════════════════════════════════════════════════════════════╝${NC}"
        echo ""
        echo -e "  ${DIM}Если что-то не работает — смотри логи первым делом. Там видны${NC}"
        echo -e "  ${DIM}ошибки конфигурации, переключения туннелей, подключения клиентов.${NC}"
        echo ""
        ...
        echo ""
        echo -e "  ${RED}99${NC})  🔴  ${WHITE}Все логи одним потоком (Ctrl+C — выход)${NC}"
        echo ""
        echo -e "  ${RED} 0${NC})  ←   Назад в главное меню"
        echo ""
        read -rp "  Введи номер: " opt
        ...
        # ── 1. WireGuard-сервер ──────────────────────────────────
        1)
            _log_header "1. 🔌  WireGuard-сервер — интерфейс, ключи, порт, пиры"
            echo -e "  ${WHITE}── journalctl wg-quick@${SERVER_WG_NIC} (последние 80 строк) ──${NC}"
            journalctl -u "wg-quick@${SERVER_WG_NIC}.service" -n 80 --no-pager 2>/dev/null \
                || echo "  (сервис не найден)"
            echo ""
            echo -e "  ${WHITE}── wg show ${SERVER_WG_NIC} ──${NC}"
            wg show "${SERVER_WG_NIC}" 2>/dev/null || echo "  (интерфейс не поднят)"
            echo ""
            echo -e "  ${WHITE}── ip addr show ${SERVER_WG_NIC} ──${NC}"
            ip addr show "${SERVER_WG_NIC}" 2>/dev/null || echo "  (интерфейс отсутствует)"
            echo ""
            echo -e "  ${WHITE}── ip link show ${SERVER_WG_NIC} ──${NC}"
            ip link show "${SERVER_WG_NIC}" 2>/dev/null || echo "  —"
            echo ""
            echo -e "  ${WHITE}── ss -lnup (UDP-порты) ──${NC}"
            ss -lnup 2>/dev/null | grep -E "WireGuard|:${SERVER_PORT}|UNCONN" | head -20 || echo "  —"
            _log_file "wg-server-trap.log" "${WG_ERR_LOG:-/var/log/wg-server-trap.log}" 40
            ;;

        # ── 2. Туннели ───────────────────────────────────────────
        2)
            _log_header "2. 🔀  Туннели — handshake, эндпоинты, трафик"
            local _ti
            if [ "${TUNNEL_COUNT:-0}" -gt 0 ]; then
                for (( _ti=0; _ti<TUNNEL_COUNT; _ti++ )); do
                    local _tiface="${TUNNEL_IFACE[${_ti}]:-}"
                    [ -z "${_tiface}" ] && continue
                    echo -e "  ${WHITE}── journalctl wg-quick@${_tiface} (последние 50 строк) ──${NC}"
                    journalctl -u "wg-quick@${_tiface}.service" -n 50 --no-pager 2>/dev/null \
                        || echo "  (сервис не найден)"
                    echo ""
                    echo -e "  ${WHITE}── wg show ${_tiface} ──${NC}"
                    wg show "${_tiface}" 2>/dev/null || echo "  (интерфейс не поднят)"
                    echo ""
                    echo -e "  ${WHITE}── wg show ${_tiface} transfer ──${NC}"
                    wg show "${_tiface}" transfer 2>/dev/null \
                        | awk '{printf "  rx: %s bytes  tx: %s bytes\n",$1,$2}' || echo "  —"
                    echo ""
                    echo -e "  ${WHITE}── wg show ${_tiface} latest-handshakes ──${NC}"
                    wg show "${_tiface}" latest-handshakes 2>/dev/null \
                        | awk -v now="$(date +%s)" '{
                            age=now-$2; h=int(age/3600); m=int((age%3600)/60); s=age%60
                            printf "  peer %s  последний handshake: %dч %dм %dс назад\n",$1,h,m,s
                          }' || echo "  (нет данных)"
                    echo ""
                done
            else
                echo -e "  ${DIM}  Туннели не настроены (TUNNEL_COUNT=0)${NC}"
            fi
            echo -e "  ${WHITE}── ip route show table main (туннельные маршруты) ──${NC}"
            ip route show 2>/dev/null | grep -E "wg|tun|tunnel" | head -20 || echo "  —"
            ;;

        # ── 3. Маршрутизация / fwmark ────────────────────────────
        3)
            _log_header "3. 🗺️   Маршрутизация / fwmark — ip rule, таблицы, default route"
            echo -e "  ${WHITE}── ip rule show ──${NC}"
            ip rule show 2>/dev/null || echo "  —"
            echo ""
            echo -e "  ${WHITE}── ip route show default ──${NC}"
            ip route show default 2>/dev/null || echo "  —"
            echo ""
            echo -e "  ${WHITE}── ip route show table all (первые 60 строк) ──${NC}"
            ip route show table all 2>/dev/null | head -60 || echo "  —"
            echo ""
            echo -e "  ${WHITE}── ip rule show table 51820 / 51821 / 5182x ──${NC}"
            ip rule show 2>/dev/null | grep -E '5182|0x100|0x200' || echo "  (нет fwmark-правил)"
            echo ""
            echo -e "  ${WHITE}── journalctl kernel WireGuard/route (последние 40 строк) ──${NC}"
            journalctl -k -n 40 --no-pager 2>/dev/null \
                | grep -iE 'wireguard|route|fwmark|rule|table' | tail -30 || echo "  (нет совпадений)"
            ;;

        # ── 4. nftables / GeoIP ──────────────────────────────────
        4)
            _log_header "4. 🔥  nftables / GeoIP — таблицы, sets russia, IPv4/IPv6"
            echo -e "  ${WHITE}── journalctl nftables (последние 60 строк) ──${NC}"
            journalctl -u nftables.service -n 60 --no-pager 2>/dev/null \
                || echo "  (сервис не найден)"
            echo ""
            echo -e "  ${WHITE}── nft list ruleset (наши таблицы wg-filter, wg-nat) ──${NC}"
            nft list ruleset 2>/dev/null \
                | grep -A 9999 'table ip wg-filter\|table ip wg-nat\|table inet wg' \
                | head -100 || nft list ruleset 2>/dev/null | head -100 || echo "  —"
            echo ""
            echo -e "  ${WHITE}── nft list sets (размеры GeoIP-сетов) ──${NC}"
            nft list sets 2>/dev/null | grep -E 'set russia|elements:|type ipv' | head -20 || echo "  —"
            echo ""
            echo -e "  ${WHITE}── journalctl update-ru-geoip / update-ru-ipset ──${NC}"
            journalctl -u update-ru-geoip.service -u update-ru-ipset.service -n 40 --no-pager 2>/dev/null \
                || echo "  (сервисы не найдены)"
            echo ""
            echo -e "  ${WHITE}── journalctl wg-geoip-load / wg-geoip ──${NC}"
            journalctl -u wg-geoip-load.service -u wg-geoip.service -n 40 --no-pager 2>/dev/null \
                || echo "  (сервисы не найдены)"
            # [fix v28.25.5] /var/lib/wg-geoip/ru.txt не создаётся скриптом —
            # данные загружаются напрямую в nft-сет @russia через update-ru-ipset.sh.
            # Проверяем реальное состояние: размер сета и кеш-файл etag.
            echo -e "  ${WHITE}── GeoIP IPv4 список: размер @russia set ──${NC}"
            local _geo4_cnt
            _geo4_cnt=$(nft list set inet wg-policy russia 2>/dev/null                 | grep -oEc '[0-9]{1,3}(\.[0-9]{1,3}){3}(/[0-9]{1,2})?(-[0-9.]+)?'                 | wc -l 2>/dev/null || echo 0)
            if [ "${_geo4_cnt:-0}" -gt 100 ] 2>/dev/null; then
                echo -e "  ${GREEN}✔ @russia загружен: ${_geo4_cnt} записей${NC}"
            else
                echo -e "  ${YELLOW}⚠ @russia пуст или мал (${_geo4_cnt:-0} записей) — запусти: /usr/local/bin/update-ru-ipset.sh${NC}"
            fi
            echo -e "  ${WHITE}── ETag кеш (update-ru-ipset.sh) ──${NC}"
            [ -f /var/cache/wg-geoip.etag ]                 && cat /var/cache/wg-geoip.etag | sed 's/^/  /'                 || echo "  (etag-файл отсутствует — первый запуск ещё не завершён)"
            ;;

        # ── 5. dnsmasq ───────────────────────────────────────────
        5)
            _log_header "5. 🌐  dnsmasq — сервис, порт :53, резолвинг, изоляция"
            echo -e "  ${WHITE}── journalctl dnsmasq (последние 80 строк) ──${NC}"
            journalctl -u dnsmasq.service -n 80 --no-pager 2>/dev/null \
                || echo "  (сервис не найден)"
            echo ""
            echo -e "  ${WHITE}── ss -lnup | grep :53 ──${NC}"
            ss -lnup 2>/dev/null | grep ':53 ' || echo "  (порт 53 не слушается)"
            echo ""
            echo -e "  ${WHITE}── /etc/dnsmasq.conf (первые 40 строк без комментариев) ──${NC}"
            grep -vE '^\s*#|^\s*$' /etc/dnsmasq.conf 2>/dev/null | head -40 \
                || echo "  (файл не найден)"
            echo ""
            echo -e "  ${WHITE}── /etc/dnsmasq.d/ ──${NC}"
            ls -la /etc/dnsmasq.d/ 2>/dev/null || echo "  (директория не найдена)"
            echo ""
            echo -e "  ${WHITE}── /etc/resolv.conf ──${NC}"
            cat /etc/resolv.conf 2>/dev/null || echo "  —"
            echo ""
            echo -e "  ${WHITE}── dig @127.0.0.1 google.com (через dnsmasq) ──${NC}"
            dig @127.0.0.1 google.com A +short +time=3 +tries=1 2>/dev/null || echo "  (нет ответа)"
            echo ""
            echo -e "  ${WHITE}── dig @127.0.0.1 yandex.ru (через dnsmasq) ──${NC}"
            dig @127.0.0.1 yandex.ru A +short +time=3 +tries=1 2>/dev/null || echo "  (нет ответа)"
            ;;

        # ── 6. unbound ───────────────────────────────────────────
        6)
            _log_header "6. 🔄  unbound — рекурсия, DNSSEC, root.hints, :5335"
            echo -e "  ${WHITE}── journalctl unbound (последние 80 строк) ──${NC}"
            journalctl -u unbound.service -n 80 --no-pager 2>/dev/null \
                || echo "  (сервис не найден)"
            echo ""
            echo -e "  ${WHITE}── ss -lntu | grep :5335 ──${NC}"
            ss -lntu 2>/dev/null | grep ':5335 ' || echo "  (порт 5335 не слушается)"
            echo ""
            echo -e "  ${WHITE}── /etc/unbound/unbound.conf.d/ ──${NC}"
            ls -la /etc/unbound/unbound.conf.d/ 2>/dev/null || echo "  (директория не найдена)"
            echo ""
            echo -e "  ${WHITE}── wg-recursive.conf / wg-unbound.conf ──${NC}"
            cat /etc/unbound/unbound.conf.d/wg-recursive.conf 2>/dev/null \
                || cat /etc/unbound/unbound.conf.d/wg-unbound.conf 2>/dev/null \
                || echo "  (конфиг не найден)"
            echo ""
            echo -e "  ${WHITE}── root.key: наличие и права ──${NC}"
            ls -la /var/lib/unbound/root.key 2>/dev/null || echo "  (root.key отсутствует — DNSSEC не будет работать)"
            echo ""
            echo -e "  ${WHITE}── root.hints: наличие ──${NC}"
            ls -lh /var/lib/unbound/root.hints 2>/dev/null || echo "  (root.hints отсутствует)"
            echo ""
            echo -e "  ${WHITE}── unbound-checkconf ──${NC}"
            unbound-checkconf 2>&1 | head -20 || echo "  (unbound-checkconf не найден)"
            echo ""
            echo -e "  ${WHITE}── dig @127.0.0.1 -p 5335 google.com ──${NC}"
            dig @127.0.0.1 -p 5335 google.com A +short +time=5 +tries=1 2>/dev/null || echo "  (нет ответа)"
            echo ""
            echo -e "  ${WHITE}── DNSSEC: dig +dnssec dnssec-tools.org (флаг ad) ──${NC}"
            dig @127.0.0.1 -p 5335 dnssec-tools.org A +dnssec +time=5 +tries=1 2>/dev/null \
                | grep -E 'status:|flags:|ad|RRSIG|NSEC' | head -10 || echo "  (нет ответа)"
            echo ""
            echo -e "  ${WHITE}── BOGUS: dig servfail.nl (должен вернуть SERVFAIL) ──${NC}"
            dig @127.0.0.1 -p 5335 servfail.nl A +time=5 +tries=1 2>/dev/null \
                | grep -E 'status:|SERVFAIL' || echo "  (нет ответа)"
            ;;

        # ── 7. DNS-over-TLS (stubby) ─────────────────────────────
        7)
            _log_header "7. 🔐  DNS-over-TLS (stubby) — :5353, TLS-сертификаты"
            echo -e "  ${WHITE}── journalctl stubby (последние 80 строк) ──${NC}"
            journalctl -u stubby.service -n 80 --no-pager 2>/dev/null \
                || echo "  (сервис не найден)"
            echo ""
            echo -e "  ${WHITE}── ss -lntu | grep :5353 ──${NC}"
            ss -lntu 2>/dev/null | grep ':5353 ' || echo "  (порт 5353 не слушается)"
            echo ""
            echo -e "  ${WHITE}── /etc/stubby/stubby.yml (без комментариев) ──${NC}"
            grep -vE '^\s*#|^\s*$' /etc/stubby/stubby.yml 2>/dev/null | head -50 \
                || echo "  (файл не найден)"
            echo ""
            echo -e "  ${WHITE}── dig @127.0.0.1 -p 5353 cloudflare.com ──${NC}"
            dig @127.0.0.1 -p 5353 cloudflare.com A +short +time=8 +tries=1 2>/dev/null \
                || echo "  (нет ответа)"
            echo ""
            echo -e "  ${WHITE}── TLS-рукопожатие 1.1.1.1:853 (Cloudflare DoT) ──${NC}"
            timeout 6 bash -c 'echo | openssl s_client -connect 1.1.1.1:853 \
                -servername cloudflare-dns.com 2>/dev/null \
                | grep -E "CONNECTED|subject|issuer|verify|SSL-Session"' \
                || echo "  (недоступен или истёк таймаут)"
            echo ""
            echo -e "  ${WHITE}── TLS-рукопожатие 9.9.9.9:853 (Quad9 DoT) ──${NC}"
            timeout 6 bash -c 'echo | openssl s_client -connect 9.9.9.9:853 \
                -servername dns.quad9.net 2>/dev/null \
                | grep -E "CONNECTED|subject|issuer|verify|SSL-Session"' \
                || echo "  (недоступен или истёк таймаут)"
            ;;
        ...
        # ── 8. DNS chain-тесты ───────────────────────────────────
        8)
            _log_header "8. 🔗  DNS chain-тесты — сквозной резолвинг, утечки, PTR, AAAA"
            echo -e "  ${WHITE}── /etc/resolv.conf ──${NC}"
            cat /etc/resolv.conf 2>/dev/null || echo "  —"
            echo ""
            echo -e "  ${WHITE}── getent hosts google.com (системный резолвер) ──${NC}"
            getent hosts google.com 2>/dev/null || echo "  (не резолвится)"
            echo ""
            echo -e "  ${WHITE}── getent hosts yandex.ru (системный резолвер) ──${NC}"
            getent hosts yandex.ru 2>/dev/null || echo "  (не резолвится)"
            echo ""
            echo -e "  ${WHITE}── dig @127.0.0.1 google.com (dnsmasq :53) ──${NC}"
            dig @127.0.0.1 google.com A +short +time=3 +tries=1 2>/dev/null || echo "  (нет ответа)"
            echo ""
            echo -e "  ${WHITE}── geo-chain: dig @127.0.0.1 -p 5335 github.com (unbound) ──${NC}"
            dig @127.0.0.1 -p 5335 github.com A +short +time=5 +tries=1 2>/dev/null \
                || echo "  (нет ответа — unbound не активен или не geo-режим)"
            echo ""
            echo -e "  ${WHITE}── PTR: dig -x 1.1.1.1 (обратный резолвинг) ──${NC}"
            dig @127.0.0.1 -x 1.1.1.1 +short +time=3 +tries=1 2>/dev/null || echo "  —"
            echo ""
            echo -e "  ${WHITE}── AAAA: dig @127.0.0.1 google.com AAAA ──${NC}"
            dig @127.0.0.1 google.com AAAA +short +time=3 +tries=1 2>/dev/null || echo "  —"
            echo ""
            echo -e "  ${WHITE}── Проверка DNS-утечки: nameserver в resolv.conf ──${NC}"
            grep '^nameserver' /etc/resolv.conf 2>/dev/null \
                | awk '{print "  nameserver:", $2, ($2=="127.0.0.1" ? "✔ локально (OK)" : "⚠ ВНЕШНИЙ (утечка?)")}'
            echo ""
            echo -e "  ${WHITE}── nft: заблокированы ли внешние DNS (dport 53) ──${NC}"
            nft list ruleset 2>/dev/null | grep -E 'dport 53|dport { 53' | head -10 || echo "  (правил нет)"
            ;;

        # ── 9. Балансировщик ─────────────────────────────────────
        9)
            _log_header "9. ⚖️   Балансировщик — wg-balance, watchdog, таблицы"
            echo -e "  ${WHITE}── journalctl wg-balance (последние 80 строк) ──${NC}"
            journalctl -u wg-balance.service -n 80 --no-pager 2>/dev/null \
                || echo "  (сервис не найден)"
            echo ""
            echo -e "  ${WHITE}── journalctl wg-balance-watchdog ──${NC}"
            journalctl -u wg-balance-watchdog.service -n 40 --no-pager 2>/dev/null \
                || echo "  (сервис не найден)"
            echo ""
            echo -e "  ${WHITE}── systemctl status wg-balance ──${NC}"
            systemctl status wg-balance.service --no-pager 2>/dev/null \
                || echo "  (сервис не найден)"
            echo ""
            echo -e "  ${WHITE}── /etc/systemd/system/wg-balance.service ──${NC}"
            cat /etc/systemd/system/wg-balance.service 2>/dev/null \
                || echo "  (unit-файл не найден)"
            echo ""
            echo -e "  ${WHITE}── ip rule show (таблицы балансировщика 5182x) ──${NC}"
            ip rule show 2>/dev/null | grep -E '5182|0x100000|0x200000' || echo "  (нет fwmark-правил)"
            ;;

        # ── 10. Anti-DPI ─────────────────────────────────────────
        10)
            _log_header "10. 🛡   Anti-DPI — BBR, MSS clamping, jitter"
            echo -e "  ${WHITE}── journalctl wg-antidpi-jitter ──${NC}"
            journalctl -u wg-antidpi-jitter.service -n 40 --no-pager 2>/dev/null \
                || echo "  (сервис не найден или jitter не включён)"
            echo ""
            echo -e "  ${WHITE}── systemctl status wg-antidpi-jitter ──${NC}"
            systemctl status wg-antidpi-jitter.service --no-pager 2>/dev/null \
                || echo "  (сервис не найден)"
            echo ""
            echo -e "  ${WHITE}── tc qdisc show (netem jitter на WG-интерфейсах) ──${NC}"
            tc qdisc show 2>/dev/null | grep -E 'netem|wg|tun' \
                || echo "  (netem не применён — jitter не активен)"
            echo ""
            echo -e "  ${WHITE}── sysctl: BBR / qdisc ──${NC}"
            sysctl net.core.default_qdisc net.ipv4.tcp_congestion_control 2>/dev/null || echo "  —"
            echo ""
            echo -e "  ${WHITE}── /etc/sysctl.d/99-antidpi.conf ──${NC}"
            cat /etc/sysctl.d/99-antidpi.conf 2>/dev/null || echo "  (файл не найден — BBR/MSS не настроен)"
            echo ""
            echo -e "  ${WHITE}── nft: MSS clamping (tcp option maxseg) ──${NC}"
            nft list ruleset 2>/dev/null | grep -i 'tcp option maxseg\|mss\|maxseg' | head -10 \
                || echo "  (правил MSS не найдено)"
            _log_file "antidpi.conf (параметры)" /etc/wireguard/.antidpi.conf 30
            ;;

        # ── 11. Killswitch ───────────────────────────────────────
        11)
            _log_header "11. 🔒  Killswitch — nft-правила, конфиги клиентов"
            echo -e "  ${WHITE}── /etc/wireguard/.killswitch-clients ──${NC}"
            if [ -f /etc/wireguard/.killswitch-clients ] && [ -s /etc/wireguard/.killswitch-clients ]; then
                cat /etc/wireguard/.killswitch-clients
            else
                echo "  (killswitch не включён ни для одного клиента)"
            fi
            echo ""
            echo -e "  ${WHITE}── nft: DROP-правила killswitch ──${NC}"
            nft list ruleset 2>/dev/null | grep -B2 -A2 -i 'drop\|kill' | head -40 \
                || echo "  —"
            echo ""
            echo -e "  ${WHITE}── Конфиги клиентов (PostUp/PreDown с iptables) ──${NC}"
            grep -l 'PostUp\|PreDown' /etc/wireguard/clients/*.conf 2>/dev/null \
                | while read -r f; do
                    echo "  Клиент: ${f}"
                    grep -E 'PostUp|PreDown' "${f}" 2>/dev/null | sed 's/^/    /'
                done || echo "  (клиентских конфигов с killswitch нет)"
            ;;

        # ── 12. Telemt (MTProxy) ─────────────────────────────────
        12)
            _log_header "12. 🦀  Telemt (MTProxy) — бинарь, сервис, API, порт, SHA256"
            echo -e "  ${WHITE}── journalctl telemt (последние 80 строк) ──${NC}"
            journalctl -u telemt.service -n 80 --no-pager 2>/dev/null \
                || echo "  (сервис не найден)"
            echo ""
            echo -e "  ${WHITE}── systemctl status telemt ──${NC}"
            systemctl status telemt.service --no-pager 2>/dev/null \
                || echo "  (сервис не найден)"
            echo ""
            echo -e "  ${WHITE}── бинарь telemt: версия и SHA256 ──${NC}"
            if command -v telemt >/dev/null 2>&1; then
                telemt --version 2>/dev/null || true
                sha256sum "$(command -v telemt)" 2>/dev/null || true
            else
                echo "  (telemt не установлен)"
            fi
            echo ""
            echo -e "  ${WHITE}── /etc/telemt/config.yml (без паролей) ──${NC}"
            grep -vE 'secret|password|token|key' /etc/telemt/config.yml 2>/dev/null \
                || grep -vE 'secret|password|token|key' /etc/telemt.yml 2>/dev/null \
                || echo "  (конфиг не найден)"
            echo ""
            echo -e "  ${WHITE}── ss: порт telemt ──${NC}"
            ss -lntp 2>/dev/null | grep -E 'telemt|8443|8888|443' | head -10 || echo "  —"
            ;;

        # ── 13. MTProto (старый mtg) ──────────────────────────────
        13)
            _log_header "13. 📡  MTProto (старый mtg) — сервис, secret, порт, ссылки"
            echo -e "  ${WHITE}── journalctl mtg (последние 80 строк) ──${NC}"
            journalctl -u mtg.service -n 80 --no-pager 2>/dev/null \
                || echo "  (сервис не найден)"
            echo ""
            echo -e "  ${WHITE}── systemctl status mtg ──${NC}"
            systemctl status mtg.service --no-pager 2>/dev/null \
                || echo "  (сервис не найден)"
            echo ""
            echo -e "  ${WHITE}── /etc/systemd/system/mtg.service ──${NC}"
            cat /etc/systemd/system/mtg.service 2>/dev/null \
                || echo "  (unit-файл не найден)"
            echo ""
            echo -e "  ${WHITE}── ссылки MTProto ──${NC}"
            cat /root/mtproto_links.txt 2>/dev/null || echo "  (файл ссылок не найден)"
            echo ""
            echo -e "  ${WHITE}── secret-токены ──${NC}"
            cat /root/mtproto_secrets.txt 2>/dev/null || echo "  (файл секретов не найден)"
            echo ""
            echo -e "  ${WHITE}── ss: порт mtg ──${NC}"
            ss -lntp 2>/dev/null | grep -E 'mtg|8443|2443' | head -10 || echo "  —"
            ;;

        # ── 14. Интернет / связь ─────────────────────────────────
        14)
            _log_header "14. 🌍  Интернет / связь — TCP, HTTPS, внешний IP, пинги"
            echo -e "  ${WHITE}── Внешний IP (curl ifconfig.me) ──${NC}"
            curl -s --max-time 5 ifconfig.me 2>/dev/null && echo "" \
                || curl -s --max-time 5 https://api.ipify.org 2>/dev/null && echo "" \
                || echo "  (нет ответа — нет интернета?)"
            echo ""
            echo -e "  ${WHITE}── ping 8.8.8.8 (4 пакета) ──${NC}"
            ping -c4 -W3 8.8.8.8 2>/dev/null || echo "  (ICMP заблокирован или нет маршрута)"
            echo ""
            echo -e "  ${WHITE}── ping 1.1.1.1 (Cloudflare, 4 пакета) ──${NC}"
            ping -c4 -W3 1.1.1.1 2>/dev/null || echo "  (ICMP заблокирован или нет маршрута)"
            echo ""
            echo -e "  ${WHITE}── TCP 8.8.8.8:53 (Google DNS) ──${NC}"
            ( exec 3<>/dev/tcp/8.8.8.8/53 ) 2>/dev/null \
                && echo "  ✔ TCP 8.8.8.8:53 доступен" \
                || echo "  ✖ TCP 8.8.8.8:53 недоступен"
            echo ""
            echo -e "  ${WHITE}── TCP 1.1.1.1:443 (Cloudflare HTTPS) ──${NC}"
            ( exec 3<>/dev/tcp/1.1.1.1/443 ) 2>/dev/null \
                && echo "  ✔ TCP 1.1.1.1:443 доступен" \
                || echo "  ✖ TCP 1.1.1.1:443 недоступен"
            echo ""
            echo -e "  ${WHITE}── curl -I https://www.google.com ──${NC}"
            curl -I -s --max-time 8 https://www.google.com 2>/dev/null | head -5 \
                || echo "  (нет ответа)"
            echo ""
            echo -e "  ${WHITE}── ip route show default ──${NC}"
            ip route show default 2>/dev/null || echo "  (default route не найден)"
            echo ""
            echo -e "  ${WHITE}── traceroute 8.8.8.8 (5 хопов) ──${NC}"
            traceroute -m 5 -w 2 8.8.8.8 2>/dev/null || mtr -n -c 1 --report 8.8.8.8 2>/dev/null \
                || echo "  (traceroute/mtr не найден)"
            ;;

        # ── 15. IPv6 ─────────────────────────────────────────────
        15)
            _log_header "15. 6️⃣   IPv6 — ядро, адреса, GeoIP, пинг6"
            echo -e "  ${WHITE}── sysctl: IPv6 включён ──${NC}"
            sysctl net.ipv6.conf.all.disable_ipv6 net.ipv6.conf.default.disable_ipv6 2>/dev/null \
                | awk -F= '{v=$2+0; print $0, (v==0?"✔ включён":"✖ отключён")}'
            echo ""
            echo -e "  ${WHITE}── ip -6 addr show ──${NC}"
            ip -6 addr show 2>/dev/null || echo "  (нет IPv6-адресов)"
            echo ""
            echo -e "  ${WHITE}── ip -6 route show default ──${NC}"
            ip -6 route show default 2>/dev/null || echo "  (нет IPv6 default route)"
            echo ""
            echo -e "  ${WHITE}── ping6 2001:4860:4860::8888 (Google, 4 пакета) ──${NC}"
            ping6 -c4 -W3 2001:4860:4860::8888 2>/dev/null \
                || echo "  (IPv6 недоступен или нет маршрута)"
            echo ""
            echo -e "  ${WHITE}── dig @127.0.0.1 google.com AAAA ──${NC}"
            dig @127.0.0.1 google.com AAAA +short +time=3 +tries=1 2>/dev/null || echo "  —"
            echo ""
            echo -e "  ${WHITE}── nft: IPv6 GeoIP (sets russia_v6) ──${NC}"
            nft list sets 2>/dev/null | grep -A3 'russia_v6' | head -15 \
                || grep -r 'russia_v6\|force_tunnel_v6' /etc/nftables.conf 2>/dev/null | head -10 \
                || echo "  (нет IPv6 GeoIP sets)"
            echo ""
            echo -e "  ${WHITE}── ip6tables -L (если есть) ──${NC}"
            ip6tables -L -n 2>/dev/null | head -30 || echo "  (ip6tables недоступен)"
            ;;

        # ── 16. Автозапуск / systemd ─────────────────────────────
        16)
            _log_header "16. 🚀  Автозапуск / systemd — enabled, verify, таймеры"
            echo -e "  ${WHITE}── systemctl list-units: наши сервисы ──${NC}"
            systemctl list-units --no-pager 2>/dev/null \
                | grep -E 'wg-|nftables|dnsmasq|unbound|stubby|mtg|telemt|update-ru' | head -30 \
                || echo "  (нет данных)"
            echo ""
            echo -e "  ${WHITE}── systemctl list-unit-files: enabled/disabled ──${NC}"
            systemctl list-unit-files --no-pager 2>/dev/null \
                | grep -E 'wg-|nftables|dnsmasq|unbound|stubby|mtg|telemt|update-ru' | head -30 \
                || echo "  (нет данных)"
            echo ""
            echo -e "  ${WHITE}── таймеры (systemctl list-timers) ──${NC}"
            systemctl list-timers --no-pager 2>/dev/null \
                | grep -E 'wg-|update-ru|geoip' | head -20 \
                || echo "  (нет WG-таймеров)"
            echo ""
            echo -e "  ${WHITE}── systemd-analyze verify wg-quick@${SERVER_WG_NIC} ──${NC}"
            systemd-analyze verify "wg-quick@${SERVER_WG_NIC}.service" 2>&1 | head -10 \
                || echo "  —"
            echo ""
            echo -e "  ${WHITE}── journalctl -b -p err (ошибки с последней загрузки) ──${NC}"
            journalctl -b -p err --no-pager -n 40 2>/dev/null \
                | grep -iE 'wg-|nftables|dnsmasq|unbound|stubby|wireguard|antidpi' | head -30 \
                || echo "  (ошибок связанных с нашими сервисами нет)"
            echo ""
        ...
        17)
            _log_header "17. 🔑  Безопасность / sysctl — forwarding, rp_filter, порты"
            echo -e "  ${WHITE}── sysctl: forwarding, rp_filter ──${NC}"
            sysctl \
                net.ipv4.ip_forward \
                net.ipv6.conf.all.forwarding \
                "net.ipv4.conf.${MAIN_INTERFACE}.rp_filter" \
                "net.ipv4.conf.${SERVER_WG_NIC}.rp_filter" \
                2>/dev/null || echo "  (ошибка sysctl)"
            echo ""
            echo -e "  ${WHITE}── /etc/sysctl.d/99-wireguard-forwarding.conf ──${NC}"
            cat /etc/sysctl.d/99-wireguard-forwarding.conf 2>/dev/null \
                || echo "  (файл не найден)"
            echo ""
            echo -e "  ${WHITE}── ss -lntu: все слушающие сокеты ──${NC}"
            ss -lntu 2>/dev/null | grep -v '127\.\|::1' | head -30 || echo "  —"
            echo ""
            echo -e "  ${WHITE}── /var/log/wg-server-trap.log (ошибки скрипта) ──${NC}"
            _log_file "wg-server-trap.log" "${WG_ERR_LOG:-/var/log/wg-server-trap.log}" 40
            echo ""
            echo -e "  ${WHITE}── /var/log/wg-audit.log (аудит действий) ──${NC}"
            _log_file "wg-audit.log" "/var/log/wg-audit.log" 40
            echo ""
            echo -e "  ${WHITE}── /etc/nftables.conf (наличие и размер) ──${NC}"
            ls -lh /etc/nftables.conf 2>/dev/null || echo "  (файл не найден)"
            echo ""
            echo -e "  ${WHITE}── journalctl -p err (ошибки ядра / firewall) ──${NC}"
            journalctl -b -p err --no-pager -n 20 2>/dev/null | head -20 || echo "  —"
            ;;

        # ── 18. Бэкапы ───────────────────────────────────────────
        18)
            _log_header "18. 💾  Бэкапы — директория, архивы, дата"
            local _bakdir="${BACKUP_DIR:-/var/backups/wg}"
            echo -e "  ${WHITE}── Директория бэкапов: ${_bakdir} ──${NC}"
            ls -lht "${_bakdir}"/*.tar.gz 2>/dev/null \
                || echo "  (архивов нет или директория отсутствует: ${_bakdir})"
            echo ""
            echo -e "  ${WHITE}── Также проверяем /root/wg-backup-*.tar.gz ──${NC}"
            ls -lht /root/wg-backup-*.tar.gz 2>/dev/null || echo "  (архивов в /root нет)"
            echo ""
            echo -e "  ${WHITE}── Содержимое последнего архива ──${NC}"
            local _last_bak
            _last_bak=$(ls -t "${_bakdir}"/*.tar.gz /root/wg-backup-*.tar.gz 2>/dev/null | head -1)
            if [ -n "${_last_bak}" ]; then
                echo "  Файл: ${_last_bak}"
                tar -tzf "${_last_bak}" 2>/dev/null | head -30 || echo "  (ошибка чтения архива)"
            else
                echo "  (нет бэкап-архивов)"
            fi
            echo ""
            echo -e "  ${WHITE}── df -h: дисковое пространство ──${NC}"
            df -h / 2>/dev/null | tail -1
            ;;

        # ── 99. Все логи в реальном времени ──────────────────────
        99)
            _log_header "🔴  Все логи в реальном времени (Ctrl+C — выход)"
            echo -e "  ${DIM}Объединённый поток всех сервисов системы.${NC}"
            echo ""
            trap 'echo ""; trap - INT' INT
            journalctl -f --no-pager \
                -u "wg-quick@${SERVER_WG_NIC}.service" \
                -u wg-balance.service \
                -u wg-balance-watchdog.service \
                -u wg-antidpi-jitter.service \
                -u nftables.service \
                -u update-ru-geoip.service \
                -u update-ru-ipset.service \
                -u wg-geoip-load.service \
                -u wg-geoip.service \
                -u dnsmasq.service \
                -u unbound.service \
                -u stubby.service \
                ...
                -u telemt.service \
                $(for _ti in "${TUNNEL_IFACE[@]:-}"; do
                    [ -n "${_ti}" ] && printf ' -u wg-quick@%s.service' "${_ti}"; done) \
                2>/dev/null
            trap - INT
            ;;
...
_tstSectionTunnels() {
    ...
    for (( i=0; i<TUNNEL_COUNT; i++ )); do
        ...
        if [ -n "${host}" ]; then
            ...
            # [fix v28.25.4] WireGuard работает поверх UDP — TCP-проверка порта эндпоинта
            # всегда даёт FAIL (провайдеры не отвечают на TCP к UDP-портам WG).
            # Заменяем на: wg handshake ≤ 3 мин (реальная проверка связи) + ICMP ping как fallback.
            _tst "Туннель ${iface}: эндпоинт ${host} достижим (wg handshake или ping)" \
                bash -c "ts=\$(wg show \"${iface}\" latest-handshakes 2>/dev/null | awk '\$2>0{print \$2}' | head -1); \
                    [ -n \"\$ts\" ] && [ \$(( \$(date +%s) - ts )) -lt 180 ] && exit 0; \
...
_tstSectionRouting() {
    ...
    _tst "Маршрут до WG-подсети на ${SERVER_WG_NIC}" \
        bash -c "ip route show dev \"${SERVER_WG_NIC}\" 2>/dev/null | grep -q . \
            || ip route show 2>/dev/null | grep -q \"dev ${SERVER_WG_NIC}\""
...
_tstSectionNftables() {
    ...
    # [fix v28.24.26] Если nftables enabled но inactive (свежий старт системы) — поднимаем автоматически,
    # аналогично тому как делается для Jitter в секции 10.
    if systemctl is-enabled nftables >/dev/null 2>&1 \
        && ! systemctl is-active nftables >/dev/null 2>&1; then
        systemctl start nftables 2>/dev/null || true
        sleep 1
        # После старта nftables нужно перезагрузить GeoIP drop-in если он есть
        if [ -f /etc/systemd/system/nftables.service.d/99-geoip.conf ]; then
            local _geoip_unit
            _geoip_unit=$(grep -oP 'Description=\K.*' /etc/systemd/system/nftables.service.d/99-geoip.conf 2>/dev/null | head -1)
            # Ищем unit файл загрузки GeoIP и запускаем его
            local _geoip_svc
            for _geoip_svc in wg-geoip-load.service wg-geoip.service update-ru-geoip.service; do
                if systemctl list-units --full --all 2>/dev/null | grep -q "${_geoip_svc}"; then
                    systemctl start "${_geoip_svc}" 2>/dev/null || true
                    break
                fi
            done
            # Прямой запуск update-ru-ipset.sh если скрипт есть
            if [ -x /usr/local/bin/update-ru-ipset.sh ] \
                && ! nft list set inet wg-policy russia >/dev/null 2>&1; then
                bash /usr/local/bin/update-ru-ipset.sh >/dev/null 2>&1 || true
            fi
        fi
    fi
...
_tstSectionUnbound() {
    ...
    # [fix v28.24.26] Если unbound enabled но не активен (свежий старт системы или
    # DNS_MODE переключён на geo) — запускаем автоматически перед тестами.
    # Убеждаемся что root.key существует — иначе unbound не стартует.
    if systemctl is-enabled unbound >/dev/null 2>&1 \
        && ! systemctl is-active unbound >/dev/null 2>&1; then
        # [fix v28.24.26] Используем _setupUnbound если доступна — она правильно
        # удаляет конфликтующий root-auto-trust-anchor-file.conf и настраивает drop-in.
        # Если _setupUnbound недоступна (вызов из одиночной секции) — минимальный fix.
        if declare -F _setupUnbound >/dev/null 2>&1; then
            _setupUnbound >/dev/null 2>&1 || true
        else
            rm -f /etc/unbound/unbound.conf.d/root-auto-trust-anchor-file.conf
            mkdir -p /etc/systemd/system/unbound.service.d
            printf '[Service]\nEnvironment=DAEMON_OPTS=\n' \
                > /etc/systemd/system/unbound.service.d/99-wireguard.conf
            systemctl daemon-reload 2>/dev/null || true
            systemctl start unbound 2>/dev/null || true
        fi
        local _wait=0
        while [ "${_wait}" -lt 8 ]; do
            ss -lntu 2>/dev/null | grep -q '127\.0\.0\.1:5335' && break
    ...
    # [fix v28.25.4] Проверяем root.key перед DNSSEC-тестами.
    # Если файл отсутствует или повреждён — пересоздаём через unbound-anchor,
    # удаляем дублирующий системный trust-anchor конфиг и перезапускаем unbound.
    if ! test -s /var/lib/unbound/root.key 2>/dev/null \
        || ! dig @127.0.0.1 -p 5335 dnssec-tools.org A +dnssec +time=3 +tries=1 2>/dev/null | grep -q 'ad'; then
        # Удаляем дублирующий системный файл (Ubuntu Noble создаёт его сам)
        rm -f /etc/unbound/unbound.conf.d/root-auto-trust-anchor-file.conf 2>/dev/null || true
        # Обновляем trust-anchor
        if command -v unbound-anchor >/dev/null 2>&1; then
            unbound-anchor -a /var/lib/unbound/root.key 2>/dev/null || true
        elif [ -x /usr/libexec/unbound-helper ]; then
            /usr/libexec/unbound-helper root_trust_anchor_update 2>/dev/null || true
        fi
        chown unbound:unbound /var/lib/unbound/root.key 2>/dev/null || true
        # Убеждаемся что auto-trust-anchor-file прописан в конфиге
        local _wcfg
        _wcfg=$(find /etc/unbound/unbound.conf.d/ -name "wg-recursive.conf" -o -name "wg-unbound.conf" 2>/dev/null | head -1)
        if [ -n "${_wcfg}" ] && ! grep -q 'auto-trust-anchor-file' "${_wcfg}" 2>/dev/null; then
            echo 'auto-trust-anchor-file: "/var/lib/unbound/root.key"' >> "${_wcfg}"
        fi
        systemctl restart unbound 2>/dev/null || true
        local _w=0
        while [ "${_w}" -lt 8 ]; do
    ...
    # [fix v28.24.26] Проверяем все возможные пути к конфигу (включая wg-recursive.conf)
    _tst "unbound.conf существует" \
        bash -c "test -f /etc/unbound/unbound.conf \
            || test -f /etc/unbound/unbound.conf.d/wg-unbound.conf \
            || test -f /etc/unbound/unbound.conf.d/wg-recursive.conf"
...
_tstSectionDoT() {
    ...
    # [fix v28.25.4] Если stubby enabled но не запущен — стартуем автоматически перед тестами
    if systemctl is-enabled stubby >/dev/null 2>&1 \
        && ! systemctl is-active stubby >/dev/null 2>&1; then
        systemctl start stubby 2>/dev/null || true
        local _sw=0
        while [ "${_sw}" -lt 8 ]; do
            ss -lntu 2>/dev/null | grep -q '127\.0\.0\.1:5353' && break
    ...
    _tst "TLS-сертификат Cloudflare 1.1.1.1:853 доступен" \
        bash -c "( timeout 6 bash -c 'echo | openssl s_client -connect 1.1.1.1:853 \
            -servername cloudflare-dns.com 2>/dev/null | grep -q CONNECTED' ) \
            || nc -zw5 1.1.1.1 853 2>/dev/null"
    _tst "TLS-сертификат Quad9 9.9.9.9:853 доступен" \
        bash -c "( timeout 6 bash -c 'echo | openssl s_client -connect 9.9.9.9:853 \
            -servername dns.quad9.net 2>/dev/null | grep -q CONNECTED' ) \
            || nc -zw5 9.9.9.9 853 2>/dev/null"
...
_tstSectionDNSChain() {
    ...
    _tst "Нет plain-text DNS наружу (порт 53 к внешним IP заблокирован нашим nft)" \
        bash -c "nft list ruleset 2>/dev/null | grep -q 'dport 53' \
            || ( nft list ruleset 2>/dev/null | grep -q 'meta l4proto.*tcp.*dport.*53\|udp dport 53\|tcp dport 53' ) \
            || ( iptables -L OUTPUT -n 2>/dev/null | grep -qE 'dpt:53.*(DROP|REJECT)' ) \
            || ( iptables -L FORWARD -n 2>/dev/null | grep -qE 'dpt:53.*(DROP|REJECT)' )"
    ...
    case "${DNS_MODE}" in
        geo)
            # [fix v28.24.27] Если unbound не слушает — вызываем _setupUnbound
            # (удаляет дублирующий trust-anchor файл, правит drop-in, стартует).
            # Голый systemctl start не починит сломанный конфиг.
            if ! ss -lntu 2>/dev/null | grep -q '127\.0\.0\.1:5335'; then
                if declare -F _setupUnbound >/dev/null 2>&1; then
                    _setupUnbound >/dev/null 2>&1 || true
                else
    ...
    if [ "${mss}" = "yes" ]; then
        _tst "MSS Clamping: правило nft для TCP MSS присутствует" \
            bash -c "nft list ruleset 2>/dev/null | grep -qi 'tcp option maxseg' \
                || nft list chain inet wg-policy mangle-forward >/dev/null 2>&1 \
                || grep -qiE 'tcp option maxseg|mangle-forward' /etc/nftables.conf 2>/dev/null"
    else
        _tst_skip "MSS Clamping: не включён"
    fi
    if [ "${jitter}" = "yes" ]; then
        # [fix v28.25.8] wg-antidpi-jitter — тип oneshot+RemainAfterExit.
        # Если юнит в состоянии failed (Exclusivity flag / другая ошибка),
        # сначала удаляем старый qdisc на всех интерфейсах, потом перезапускаем.
        # Если inactive — просто стартуем.
        local _jit_state
        _jit_state=$(systemctl is-active wg-antidpi-jitter.service 2>/dev/null || echo "unknown")
        if [ "${_jit_state}" = "failed" ]; then
            # Удаляем qdisc на всех WG интерфейсах (исправляем Exclusivity flag)
            for _jif in $(ip link show 2>/dev/null | awk -F': ' '/wg-/{print $2}'); do
                tc qdisc del dev "${_jif}" root 2>/dev/null || true
            done
            systemctl reset-failed wg-antidpi-jitter.service 2>/dev/null || true
            systemctl start wg-antidpi-jitter.service 2>/dev/null || true
            sleep 1
        elif systemctl is-enabled wg-antidpi-jitter.service >/dev/null 2>&1 \
            && [ "${_jit_state}" != "active" ]; then
            systemctl start wg-antidpi-jitter 2>/dev/null || true
            sleep 1
        fi
        _tst "Jitter: systemd-юнит wg-antidpi-jitter активен" \
            systemctl is-active wg-antidpi-jitter
        # [fix v28.25.4] Дополнительно проверяем реальное применение через tc qdisc
        # (юнит oneshot может быть active но tc qdisc слететь после ребута WG-интерфейса).
        _tst "Jitter: tc netem qdisc применён хотя бы на одном WG-интерфейсе" \
            bash -c "tc qdisc show 2>/dev/null | grep -q 'netem'"
...
_tstSectionIPv6() {
    ...
    _tst "GeoIP IPv6 fwmark-правила присутствуют в nftables" \
        bash -c "nft list ruleset 2>/dev/null | grep -q 'ip6 daddr\|ip6 saddr\|russia_v6\|meta l4proto.*ip6' \
            || grep -q 'ip6 daddr\|russia_v6\|ip6 saddr' /etc/nftables.conf 2>/dev/null"
}
...
_tstSectionSecurity() {
    ...
    _tst "Ошибок в /var/log/wg-errors.log нет (пуст или отсутствует)" \
        bash -c "log=\"${WG_ERR_LOG:-/var/log/wg-server-trap.log}\"; \
            [ ! -f \"\${log}\" ] || [ ! -s \"\${log}\" ] || { \
            bad=\$(grep -vE ':_tst[A-Za-z0-9_]*:' \"\${log}\" 2>/dev/null \
                | grep -cvE '(read|_pause|ask|askYesNo|grep|awk|wc|test|true|false|head|tail|printf|echo|cat|ls|stat|bash|sh |systemctl|ip |wg |ss |dig|nft|\[|val=|timeout|sleep|mkdir|chmod|touch|rm |cp |mv |find|sort|uniq|sed |tr |cut |xargs)' 2>/dev/null || true); \
            [ \"\${bad:-0}\" -eq 0 ]; }"
    ...
    _tst "Нет лишних открытых портов наружу (кроме WG-порта ${SERVER_PORT})" \
        bash -c "cnt=\$(ss -lntu 2>/dev/null | awk 'NR>1{print \$5}' \
            | grep -vE '127\.|::1|0\.0\.0\.0:${SERVER_PORT}|0\.0\.0\.0:10050' \
            | grep -vE '0\.0\.0\.0:[3-9][0-9]{4}' \
            | grep -c '0\.0\.0\.0:' || true); [ \"\${cnt}\" -le 6 ]"
...
menu() {
    loadConfig
    # [fix v28.25.8] Авто-восстановление MSS chain при запуске меню.
    # nftables перезапускает таблицу wg-policy из /etc/nftables.conf, но если
    # mangle-forward chain не был туда сохранён (или nftables.conf устарел),
    # цепочка пропадает. Тихо восстанавливаем если ANTIDPI_MSS=yes.
    _antidpiLoad 2>/dev/null || true
    if [ "${ANTIDPI_MSS:-no}" = "yes" ]; then
        if ! nft list chain inet wg-policy mangle-forward >/dev/null 2>&1; then
            # chain пропала — восстанавливаем без вывода (фоновое восстановление)
            nft add chain inet wg-policy mangle-forward \
                '{ type filter hook forward priority mangle; policy accept; }' 2>/dev/null || true
            nft flush chain inet wg-policy mangle-forward 2>/dev/null || true
            nft add rule inet wg-policy mangle-forward \
            ...
            # Сохраняем восстановленное состояние
            {
                flock -w 5 200 || true
                local _mss_tmp
                _mss_tmp=$(mktemp /etc/nftables.conf.XXXXXX 2>/dev/null) && \
                _saveWgNftables "${_mss_tmp}" 2>/dev/null && \
                chmod 644 "${_mss_tmp}" 2>/dev/null && \
                mv -f "${_mss_tmp}" /etc/nftables.conf 2>/dev/null || \
                rm -f "${_mss_tmp}" 2>/dev/null
            } 200>/var/run/wg-nft.lock 2>/dev/null || true
        fi
    fi
    # [fix v28.25.8] Авто-восстановление Jitter: если юнит enabled но netem слетел
    # (например, после wg-quick down/up пересоздаётся интерфейс и теряет qdisc),
    # запускаем юнит заново тихо.
    if [ "${ANTIDPI_JITTER:-no}" = "yes" ]; then
        if systemctl is-enabled wg-antidpi-jitter.service >/dev/null 2>&1; then
            if ! tc qdisc show 2>/dev/null | grep -q netem; then
                systemctl start wg-antidpi-jitter.service 2>/dev/null || true
            fi
        fi
    ...
    while true; do
        ...
        echo -e "  ${YELLOW}13${NC})  📄  ${WHITE}Логи${NC}                  ${DIM}— журналы всех 18 подсистем${NC}"
...
if [ ! -f "${CONFIG_FILE}" ]; then
    ...
    # [fix v28.25.8] Авто-пересоздаём wg-balance.sh если он устарел.
    # Признак старой версии: отсутствует маркер "# wg-balance v28.25." (версионный тег),
    # ИЛИ содержит "local time\b" (старое имя переменной — bash builtin → unbound variable).
    # Маркер надёжнее чем grep по BALANCE_INTERVAL:- который мог присутствовать в старых версиях.
    if [ -f /usr/local/bin/wg-balance.sh ]; then
        if ! grep -q '# wg-balance v28\.25\.' /usr/local/bin/wg-balance.sh 2>/dev/null \
            || grep -qE '\blocal[[:space:]]+time[[:space:]]*[;=$]' /usr/local/bin/wg-balance.sh 2>/dev/null \
            || ! grep -q 'BALANCE_INTERVAL:-' /usr/local/bin/wg-balance.sh 2>/dev/null; then
            loadConfig
            createBalanceScript
            systemctl restart wg-balance.service 2>/dev/null || true
        fi
